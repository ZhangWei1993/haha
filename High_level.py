...
class HDueling:
    ...
    def _build_net(self):
        #the s is the state, containing the output of the low level agent 
        def dqn_layers(s,net_param):
            #choose the network type as cnn
            if self.cnn:
                # [-1,50]must change according to the real size of the input state
                s = tf.reshape(s,[-1,12])
                self.input_image = s
    ...
    #                        outputs_collections=net_param,
            ...
            if self.dueling:
                ...
                with tf.variable_scope('Value'):
                    fc1_val = tf.contrib.layers.fully_connected(
                        inputs = fc0,
...
#                        outputs_collections=net_param,
        ...
        # ------------------ build evaluate_net ------------------
        self.s = tf.placeholder(tf.float32, [None, 12], name='s')
...
#            self.depth_loss1 = tf.reduce_mean(tf.squared_difference(self.laser, self.regu1))
        ...
        # ------------------ build target_net ------------------
        self.s_ = tf.placeholder(tf.float32, [None, 12], name='s_')
        # input
    ...
    def remember(self, state, action, reward, next_state,done):
        ...
        if len(self.memory) > self.memory_size:
            self.memory.popleft()
    '''
    plot the hidden represetation of the input image in the CNN.
    '''
