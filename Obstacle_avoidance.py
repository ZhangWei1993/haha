...
#                            outputs_collections=None,
                    ...
                    fc0_dr = tf.reshape(conv1, [-1, 16 * 25])
...
#                            scope=None
#                        )
#                    fc0_dr = tf.contrib.layers.dropout(
#                            fc0,
#                            keep_prob=self.rate,
#                            noise_shape=None,
#                            is_training=True,
...
#                        outputs_collections=net_param,
                    ...
                    fc2_val_dr = tf.contrib.layers.dropout(
                            fc2_val,
                            keep_prob=self.rate,
                            noise_shape=None,
                            is_training=True,
...
#                        outputs_collections=net_param,
                    ...
                    fc1_adv_dr = tf.contrib.layers.dropout(
                            fc1_adv,
                            keep_prob=self.rate,
                            noise_shape=None,
                            is_training=True,
...
#                        outputs_collections=net_param,
        ...
        # ------------------ build evaluate_net ------------------
        self.s = tf.placeholder(tf.float32, [None, 100], name='s')
...
#            self.depth_loss1 = tf.reduce_mean(tf.squared_difference(self.laser, self.regu1))
        ...
        # ------------------ build target_net ------------------
        self.s_ = tf.placeholder(tf.float32, [None, 100], name='s_')
...
#        plotNNFilter(conv2_show)
    def output(self, state,target):
        Val, Adv,act_values = self.sess.run([self.Val,self.Adv,self.q_next],feed_dict={self.s_:state,self.target_:target})
...
#        minibatch = random.sample(self.memory, self.batch_size)
#        state, target, action, reward, next_state,next_target,done = map(np.array, zip(*minibatch))
#        state = np.reshape(state,(self.batch_size,100))
#        next_state = np.reshape(next_state,(self.batch_size,100))
#        target = np.reshape(target,(self.batch_size,2))
#        next_target = np.reshape(next_target,(self.batch_size,2))
    #        for state, target, action, reward, next_state,next_target,done in minibatch:
