...
#    train_steps = 1000000
    ...
    with tf.variable_scope('dueling'):
        dueling_DQN = DuelingDQN(
            ...
            e_greedy_decay=0.999, sess=sess, dueling=True, cnn = True, fcn=None, output_graph=True)    
...
#    dueling_DQN.memory,ep = load_memory(episode=25500)#reload memory
    ...
    saver.restore(sess,"/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/four/"+str(start)+".ckpt")#reload parameters
    dueling_DQN.memory,ep = load_memory(episode=1500)#reload memory
    dueling_DQN.epsilon = float(ep)
...
#                done = True
        ...
        if e % 500 == 0:            
            save_path = saver.save(sess, "/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/four/"+str(e)+".ckpt")
            np.save('total_reward.npy',treward)
        if total_reward > -5:
            save_path = saver.save(sess, "/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/four/good/"+str(e)+".ckpt")
        if e % 1500 == 0:
