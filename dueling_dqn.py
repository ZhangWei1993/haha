...
def save_memory(memory,episode,epsilon):
    pickle.dump(memory, open("/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/for/memory/save_memory_"+str(episode)+".txt", 'wb'))
    np.save("/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/for/memory/save_epsilon"+str(episode)+".npy",epsilon)
#save the memory reply data after a number of episode in case of the training process break down
def load_memory(episode):
    memory=pickle.load(open("/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/for/memory/save_memory_"+str(episode)+".txt", 'rb'))
    ep = np.load("/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/for/memory/save_epsilon"+str(episode)+".npy")
    return memory, ep
...
#    train_steps = 1000000
    ...
    with tf.variable_scope('dueling'):
        dueling_DQN = DuelingDQN(
            action_size=ACTION_SPACE, state=state_size, epsilon=0.1, memory_size=MEMORY_SIZE,
...
#    dueling_DQN.memory,ep = load_memory(episode=25500)#reload memory
    ...
    saver.restore(sess,"/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/for/"+str(8000)+".ckpt")#reload parameters
...
#                done = True
        ...
        if e % 500 == 0:            
            save_path = saver.save(sess, "/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/for/"+str(e)+".ckpt")
            np.save('total_reward.npy',treward)
        if total_reward > -5:
            save_path = saver.save(sess, "/home/zhw/gym-gazebo/examples/turtlebot/dueling_dqn/training/images/for/good/"+str(e)+".ckpt")
        if e % 1500 == 0:
