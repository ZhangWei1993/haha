...
#    train_steps = 1000000
    ...
    with tf.variable_scope('dueling'):
        dueling_DQN = DuelingDQN(
            action_size=ACTION_SPACE, state=state_size, epsilon=0.01, memory_size=MEMORY_SIZE,
            e_greedy_decay=0.9992, sess=sess, dueling=True, cnn = True, fcn=None, output_graph=True)    
...
#        state = np.stack((state,state),axis=3)    
        ...
        for time in range(500):
            # env.render()
            action = dueling_DQN.act(state,target,"boltzmann")
