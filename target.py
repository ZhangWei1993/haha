...
class DuelingDQN:
    ...
    def _build_net(self):
        def dqn_layers(target,net_param):
...
#        self.laser = tf.placeholder(tf.float32,[None,4 * 4*16],name = 'laser')
        ...
        with tf.variable_scope('eval_net'):
            self.q_eval = dqn_layers(self.target,['eval_net_params',tf.GraphKeys.GLOBAL_VARIABLES])
...
#            self.depth_loss1 = tf.reduce_mean(tf.squared_difference(self.laser, self.regu1))
        ...
        with tf.variable_scope('target_net'):
            self.q_next = dqn_layers(self.target_,['target_net_params',tf.GraphKeys.GLOBAL_VARIABLES])

    def remember(self, target, action, reward, next_target,done):
        self.memory.append((target, action, reward, next_target ,done))
...
#        plotNNFilter(conv2_show)
    def output(self, target):
        Val, Adv,act_values = self.sess.run([self.Val,self.Adv,self.q_next],feed_dict={self.target_:target})
        return np.concatenate((Val, Adv),axis=1),np.argmax(act_values[0])
    def act(self, target,exploration):
        if exploration == "e_greedy":
            if np.random.rand() <= self.epsilon:
                return random.randrange(self.action_size)
            act_values = self.sess.run(self.q_eval, feed_dict={self.target:target})
...
#            print("Value: {}, Advantage: {}" .format(Adv,Val))
        ...
        if exploration == "boltzmann":
            act_values = self.sess.run(self.q_dist, feed_dict={self.target:target, self.temp: self.epsilon})
    ...
    def learn(self):
        ...
        for target, action, reward, next_target,done in minibatch:
            q_next = self.sess.run(self.q_next, feed_dict={self.target_: next_target}) # next observation
            q_eval = self.sess.run(self.q_eval, {self.target: target})
